...
Draft `PROJECT.md` and a vertical `ROADMAP.yaml`, then ask for confirmation before writing them. Preserve existing product documentation unless the user authorizes replacement. After confirmation, write the artifacts, run `node .riff/bin/riff.mjs wave sync`, and stop.
