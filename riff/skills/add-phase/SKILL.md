...
Add one vertical, demonstrable phase with only real dependencies, blocking edges, risks, sensitivity, and exclusions. Do not renumber completed phases or rewrite foreign roadmap content unnecessarily. Run `node .riff/bin/riff.mjs wave sync` and report when the phase becomes ready.
