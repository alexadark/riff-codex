...
Use the fixed phase columns `Todo`, `In progress`, `Done`, `Blocked`, and `Skipped`. For RIFF Codex projects, render the current `.riff-state/state.json` status rather than the static roadmap status, and show a priority badge only when the roadmap declares one explicitly.
