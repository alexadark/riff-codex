...
3. Read `.riff-codex/references/evidence.md`. Stage the complete candidate, then execute scoped validation with `wave validate --run` and explicit paths. For changed UI journeys, capture real screenshots and attach the verification manifest; link the generated HTML report in the result. Use `wave retry` only for its recorded formal failure, under the contract's retry limit.
