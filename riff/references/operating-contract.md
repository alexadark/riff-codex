...
- `$riff:evolve` plans authorized changes to an onboarded application under [evolution](evolution.md). It reuses the existing product baseline and discovery gates, protects active and historical contracts, and stops before implementation. Map and onboard remain explicit prerequisites for bringing an unfamiliar brownfield application into RIFF. Pending replacement and deferral use the supported roadmap rules, never invented lifecycle states.
